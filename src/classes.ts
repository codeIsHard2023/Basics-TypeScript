...
class CollectionWithThisReturn<Type> {
  constructor(private items: Type[]) {}
  add(item: Type): this {
...
function getX(p: PointCoordinate) {
  return p.x;
}
...
abstract class Geometry {
  x = 0;
  y = 0;
  abstract surface(): number; // methode surface() est abstraite et dans ce cas c'est l'enfant qui hérite de Geometry va devoir implementer la methode surface()
}
